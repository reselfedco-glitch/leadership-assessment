...
function ActionsTab({ name, blockScores, onRestart, email: initialEmail, leadershipScore }: { 
  name: string; 
  blockScores: BlockScore[];
  onRestart: () => void;
  email: string;
  ...
  const handleEmailResults = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/send-results', {
        ...
        body: JSON.stringify({
          to: emailInput,
          subject: `Your Leadership Identity Assessment Results`,
          data: resultsData,
        }),
      })
      ...
      <Card className="border-border">
        <CardContent className="p-6">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              <p className="text-sm text-muted-foreground">Email to yourself</p>
              <div className="flex gap-2">
                ...
                <Button 
                  ...
                  disabled={!emailInput || emailSending || emailSent}
                  className="gap-2 shrink-0"
                >
                  {emailSent ? <><CheckCircle2 className="w-4 h-4" />Sent</> : emailSending ? "..." : <><Mail className="w-4 h-4" />Send</>}
                </Button>
              </div>
            </div>
          </div>
...
export function Results({ name, email, leadershipScore, blockScores, onRestart }: ResultsProps) {
  ...
  // Save results to database and send emails on mount
  useEffect(() => {
    const saveAndNotify = async () => {
      ...
      try {
        // Send to Reselfed
        const reselfedResponse = await fetch('/api/send-results', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            to: 'reselfed.co@gmail.com',
            subject: `Leadership Assessment Results - ${name}`,
            data: resultsData,
          }),
        })
        
        if (reselfedResponse.ok) {
          sentEmails.push('reselfed.co@gmail.com')
        }

        // Send to user if they provided email
        if (email) {
          const userResponse = await fetch('/api/send-results', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              to: email,
        ...
        setEmailSentTo(sentEmails)
        setEmailStatus('sent')
      } catch (error) {
        console.error('Failed to send auto emails:', error)
        setEmailStatus('error')
      }
    }

    saveAndNotify()
  }, [name, email, leadershipScore, blockScores, emailStatus])
    ...
    <div className="space-y-6">
        ...
        <div className="bg-primary/10 border border-primary/30 rounded-lg p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">
              {email 
                ? `A copy has been emailed to ${email} and to your coach at Reselfed.`
                : `A copy has been sent to your coach at Reselfed.`}
            </p>
          </div>
          <CheckCircle2 className="w-5 h-5 text-primary flex-shrink-0" />
        </div>
      ...
      {emailStatus === 'pending' && (
        <div className="bg-secondary/50 border border-border rounded-lg p-4 flex items-center gap-3">
          <Mail className="w-5 h-5 text-muted-foreground flex-shrink-0 animate-pulse" />
          <p className="text-sm text-muted-foreground">Sending your results...</p>
        </div>
      )}
      ...
      {emailStatus === 'error' && (
        <div className="bg-destructive/10 border border-destructive/30 rounded-lg p-4 flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-foreground">
              Could not send results automatically
            </p>
            <p className="text-xs text-muted-foreground">
              You can still download your results as PDF or email them manually from the Next Steps tab.
            </p>
          </div>
        </div>
      )}
...
{activeTab === "perception" && (
  <PerceptionTab name={name} blockScores={blockScores} leadershipScore={leadershipScore} />
        ...
        {activeTab === "actions" && (
          <ActionsTab name={name} blockScores={blockScores} onRestart={onRestart} email={email} leadershipScore={leadershipScore} />
        )}
      </div>
